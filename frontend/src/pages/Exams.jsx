...
import { BookOpen, ChevronDown, ChevronUp, Loader2, Plus, Upload, X } from 'lucide-react'
import { api } from '../api'
    ...
    <div className="p-6 space-y-4 max-w-4xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {exams.map(exam => (
            <div key={exam.id} className="bg-gray-900 border border-gray-800 rounded-xl p-4 flex items-center justify-between gap-4 hover:border-gray-700 transition-colors">
              <div>
                <p className="text-sm font-medium text-white">{exam.name}</p>
                <p className="text-xs text-gray-500 mt-0.5">{exam.question_count} questions · slug: {exam.slug}</p>
              </div>
              <button onClick={() => setUploadExam(exam)}
                className="shrink-0 flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg bg-gray-700 hover:bg-gray-600 text-gray-300 transition-colors">
                <Upload className="w-3.5 h-3.5" /> Upload Bank
              </button>
            </div>
          ))}
        </div>
